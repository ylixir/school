...
\begin{lemma}
  Given $A=\{n_1,\dots,n_k:n_i\in \mathbb{N}\}$ with $\gcd(A)=1$, the set of linear
...
\begin{exa}
  ...
  is a NSG.
\end{exa}
We note that while the elements of the generating set for a NSG are required to have a greatest common denominator of 1, any subset of the generating set need not meet that restriction. This means that a subsemigroup of an NSG may not actually be an NSG.
\begin{exa}
...
The complex numbers are a natural extension of the real numbers as the Gaussian integers are a natural extension of $\mathbb{Z}$. We could consider the corresponding extension of $\mathbb{N}$. In this section, we will explore the NSG construction in terms of the Gaussian integers and discuss the similarities and differences.

Let us take some ``Gaussian semigroup'' $S=\langle x_1+y_1i,\dots,
x_n+{y_n}i\rangle$ where $x_i,y_i\ge 0$. Notice that as a set, this semigroup is the direct sum of two NSGs. Say $S=\langle A\rangle\oplus\langle B\rangle=\langle x_1,\dots,x_n\rangle\oplus\langle y_1,\dots,y_n\rangle$.
