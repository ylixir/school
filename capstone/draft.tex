...
\author{Jon Allen}
\begin{abstract}
The purpose of this paper is to explore numerical semigroups. We will cover bijections between numerical semigroups, Markov bases, and lattice ideals. We will go on to investigate numerical semigroups in natural extensions of $\mathbb{N}$.
\end{abstract}
\maketitle
\section{Introduction}
There is a simple construct called a numerical semigroup. This semigroup has proven to be interesting as it is closely related to more complicated objects. In order to understand these objects we will need to use tools from many  branches of mathematics. These include statistics, abstract algebra, linear algebra, combinatorics and so on.

Because all of these disciplines will be represented we will endeavor to keep our notation consistent. For example, we will seek to use the letter $S$ to represent a numerical semigroup. The letter $A$ will be used to represent the generating set of $S$ and the letters $m$ and $M$ will represent $\min(A)$ and $\max(A)$ respectively.
...
Once we have dealt with Markov bases we will have essentially created a map from lattice ideals to semigroups in $\mathbb{N}$ and back again. This achieves our goal of using these humble semigroups to consider more complicated objects.

We are then free to imagine what else we can achieve with these objects. We will began exploring natural extensions of $\mathbb{N}$ and there we will see some surprising results. First though, we need to see the how and why for a definition of the numerical semigroup.
